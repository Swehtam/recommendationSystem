...
class BiclusterRecom():
    ...
    def __init__(self):
        ...
        get_adjacency_list(outputname)
        self.biclusters = get_biclusters()
    ...
    def get_adjacency_list(self, txt_file_name):
        if (path.exists(txt_file_name)):
            ...
            self.n_clientes = text_file.readline()
            self.n_produtos = text_file.readline()
            self.convert_cliente = pickle.load( open( "/pickle/convert_cliente.pickle", "rb" ) )
            self.revert_cliente = pickle.load( open( "/pickle/revert_cliente.pickle", "rb" ) )
            self.convert_produto = pickle.load( open( "/pickle/convert_produto.pickle", "rb" ) )
            self.revert_produto = pickle.load( open( "/pickle/revert_produto.pickle", "rb" ) )
    ...
    def get_biclusters(self):
        if (path.exists("/pickle/biclusters.pickle")):
            self.biclusters = pickle.load( open( "/pickle/biclusters.pickle", "rb" ) )
            self.clientes_cluster = pickle.load( open( "/pickle/clientes_cluster.pickle", "rb" ) )
            self.produtos_cluster = pickle.load( open( "/pickle/produtos_cluster.pickle", "rb" ) )
    ...
    def create_biclusters(self):
        ...
        pickle.dump(self.clientes_cluster, open("/pickle/clientes_cluster.pickle", "wb"))
        pickle.dump(self.produtos_cluster, open("/pickle/produtos_cluster.pickle", "wb"))
        pickle.dump(self.biclusters, open("/pickle/biclusters.pickle", "wb"))
    
    #Cria a lista adjacente e criar o arquivo txt
    def create_adjacency_list(self):
        #Ler arquivo de vendas
        df_compras = pd.read_excel('dados_vendas.xlsx')
        ...
        pickle.dump(self.convert_cliente, open("/pickle/convert_cliente.pickle", "wb"))
        pickle.dump(self.revert_cliente, open("/pickle/revert_cliente.pickle", "wb"))
        ...
        pickle.dump(self.convert_produto, open("/pickle/convert_produto.pickle", "wb"))
        pickle.dump(self.revert_produto, open("/pickle/revert_produto.pickle", "wb"))
    ...
    def execute_terminal_command(self):
        ...
        cmd = 'g++ -o ARMPB src/*.cpp'   # arbitrary external command, e.g. "python mytest.py"
